...
def ufo2mf(destPath, targetPath=None):
    if targetPath is None:
        f = CurrentFont()
    else:
        f = OpenFont(targetPath, showInterface=False)
    ...
    glyphList = [f for f in os.listdir(DIR_UFO) if split_file_name(f) == 'glif']
    fontWidth = get_font_width(DIR_UFO, glyphList)
    for glyph in glyphList:
        glyph2mf(glyph, DIR_UFO, DIR_METAFONT_RADICAL, DIR_METAFONT_COMBINATION, fontWidth)
...
def glyph2mf(glyphName, dirUFO, dirRadical, dirCombination, fontWidth):
	...
	if len(components) != 0:
		...
		# *** Changed *** #
		# fp.write('def ' + num2char(glyphName) + '(expr moveSizeOfH, moveSizeOfV) =\n') #!!!!!!!!!!!!!!!
		fp.write('def ' + num2char(glyphName) + '(expr Width, Height, moveSizeOfH, moveSizeOfV, penWidthRate, penHeightRate, curveRate, serifWidth, serifHeight) =\n')
